...
> Repo guide for AI coding agents. Source of truth lives in `docs/`. Keep this updated as patterns emerge (WORKFLOW.md §1.8).

## What this is
CustomerPortal365: a multi-tenant B2B SaaS platform for BusinessValue365 — admin console + customer self-service portal + per-product micro-sites. Full specs in `docs/` (PRD, TDD, DESIGN_SYSTEM, WORKFLOW, PROMPT_PLAYBOOK, adr/). **Requirement IDs in the PRD are binding.** PRD wins on behavior; TDD wins on mechanism.
